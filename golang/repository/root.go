package repository // 데이터베이스와 상호작용할 때 사용하는 인터페이스 제공
...
func (s *Repository) ServerSet(ip string, available bool) error {
	_, err := s.db.Exec("INSERT serverInfo(`ip`, `available`) VALUES (?, ?) ON DUPLICATE KEY UPDATE `available` = VALUES(`available`)", ip, available)
	return err
}
...
func (s *Repository) Room(name string) (*schema.Room, error) {
	...
	err := s.db.QueryRow(qs, name).Scan(&d.ID, &d.Name, &d.CreateAt, &d.UpdatedAt)
